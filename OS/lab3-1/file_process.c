...
int process_chunk(FILE* src_file, FILE* dest_file, size_t offset, size_t chunk,
    ...
    size_t total_read = 0;
    int return_code = read_chunk(src_file, buffer, chunk, &total_read, src_name);
    ...
    return_code = write_chunk(dest_file, buffer, chunk, &total_written, dest_name);
    if (return_code == EXIT_FAILURE) {
        return EXIT_FAILURE;
    }
...
int copy_reverse_file(const char* src, const char* dest) {
    FILE* src_file = NULL;
    FILE* dest_file = NULL;
    int return_code = init_files(src, dest, &src_file, &dest_file);
    if (return_code == EXIT_FAILURE) {
        return EXIT_FAILURE;
    }
    ...
    while (offset > 0) {
        ...
        offset -= chunk;
        return_code = process_chunk(src_file, dest_file, offset, chunk, src, dest);
        if (return_code == EXIT_FAILURE) {
            fclose(src_file);
            fclose(dest_file);
            return EXIT_FAILURE;
